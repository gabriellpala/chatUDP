...
class UDPReceiver implements Receiver {
	...
	public UDPReceiver(int portNumber, int bufferSize,
		...
		try {
			...
			throw new ChatException("Houve algum erro ao iniciar o receiver.",
					socketException);
		}
	...
	private void validateAttributes(int portNumber, int bufferSize,
		...
		if (portNumber <= MIN_PORT_NUMBER)
			throw new IllegalArgumentException(
					"O receiver n ̃ao pode usar portas reservadas.");
		if (container == null)
			throw new IllegalArgumentException(
					"O container de mensagens n ̃ao pode ser nulo.");
		if (bufferSize < MIN_BUFFER_SIZE)
			throw new IllegalArgumentException(String.format(
					"O tamanho do buffer precisa ser maior que %d",
					MIN_BUFFER_SIZE));
	}
